...
const AddRecipeButton = ({ url, user, authenticated, id, setUserRecipes }) => {
    ...
    return (
        <>
            {added ? (<button className='float-right btn btn-secondary btn-sm' disabled>&nbsp;&#10003;&nbsp;</button>) : (
                <>{authenticated ? (
                    <button className='float-right btn btn-success btn-sm' onClick={addToProfile}>&nbsp;+&nbsp;</button>
                ) : (<span></span>)}</>
            )}
        </>
    );
